...
describe('Login Router', () => {
  test('Should retun 400 if no email is provided', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
  ...
  test('Should retun 400 if no password is provided', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
  ...
  test('Should retun 500 if no httpRequest is provided', async () => {
    const { sut } = makeSut()
  ...
  test('Should retun 500 if httpRequest has no body', async () => {
    const { sut } = makeSut()
    const httpResponse = await sut.route({})
    expect(httpResponse.statusCode).toBe(500)
    expect(httpResponse.body).toEqual(new ServerError())
